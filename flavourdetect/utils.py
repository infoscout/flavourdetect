...
def flavour_detect(request):
    ...
    if 'iphone' in user_agent: return Flavour.IPHONE
    if 'ipad' in user_agent: return Flavour.IPAD
    if 'ipod' in user_agent: return Flavour.IPOD
    if 'android' in user_agent: return Flavour.ANDROID
    if 'blackberry' in user_agent: return Flavour.BLACKBERRY
...
class FlavourTemplateResponseMixin(TemplateResponseMixin):
    """
    Wrapper for TemplateResponseMixIn class. Appends
    flavour templates
    """
